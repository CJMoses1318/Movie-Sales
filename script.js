...
const searchValueFromParams = params.get("search");

if (!!searchValueFromParams) {
    fetchMovies(searchValueFromParams);
    input.value = searchValueFromParams;
}
...
function fetchMovies(searchFromParams) {
  const query = input.value;
  const apiUrl = `https://www.omdbapi.com/?apikey=58ad07&s=${encodeURIComponent(
    query || searchFromParams
  )}`;
...
function displayResults(data) {
  ...
  data.Search.forEach((movie) => {
    ...
    movieDiv.innerHTML = `
            ...
            <img src="${movie.Poster ? movie.Poster : "./assets/noimagefound.png "}" alt="${movie.Title} Poster">
